...
// Admin credentials
const adminName = 'zeus';

// Target users who can see full content
const targetUsers = ['sam', 'samantha'];
...
function loadUserData() {
    ...
    const savedInbox = loadFromStorage('userInbox_' + username);
    const savedCustomCodes = loadFromStorage('customRedeemCodes');
    const savedMusicData = loadFromStorage('customMusicData');
    const savedBgColor = loadFromStorage('bgColor');
    ...
    if (savedInbox) userInbox = savedInbox;
...
function registerUser(name) {
    if (!allUsers.find(u => u.name.toLowerCase() === name.toLowerCase())) {
        const user = {
            ...
            isTarget: targetUsers.includes(name.toLowerCase())
        };
        allUsers.push(user);
...
function resetMusic() {
    ...
    saveToStorage('customMusicData', null);
...
function startGame() {
    ...
    if (username.toLowerCase() === adminName) {
        ...
        isAdmin = false;
        const isTarget = targetUsers.includes(username.toLowerCase());
...
function updateUserList() {
    ...
    container.innerHTML = allUsers.map(user => `
        <div class="user-item">
            ...
            ${user.isTarget ? '<span class="user-badge-inline">⭐ Target</span>' : '<span class="user-badge-inline" style="background: #936fb5;">👤 Guest</span>'}
            <br>
...
function changeBackgroundColor() {
    ...
    saveToStorage('bgColor', gradient);
...
function resetBackgroundColor() {
    ...
    saveToStorage('bgColor', defaultGradient);
        ...
        musicUpload.addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                reader.onload = function(event) {
                    ...
                    saveToStorage('customMusicData', customMusicData);
